...
/*   Updated: 2023/10/12 16:09:35 by hdupire          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_metachar(char *line)
{
	...
	if ((line[0] == '<' || line[0] == '>') && line[1] == '&')
		...
		return (0);
	else if (is_strict_meta(line[i]) && is_metachar(line[i + 1])
		&& line[i] != line[i + 1])
		return (syntax_error(line + i + 1, 1));
	while (is_strict_meta(line[i]))
		i++;
	while (is_separator(line[i]))
		i++;
	if (is_strict_meta(line[i]))
...
int	redir_ok(char *line)
{
	...
	if (line[0] == '>' && line[1] == '<')
		...
		return (syntax_error(line + 2, 1));
	else if (line[0] == line[1] && line[1] == line[2] && line[0] == '>')
		return (syntax_error(line + 2, 1));
	i = 0;
	while (line[i] == '<' || line[i] == '>')
		i++;
	while (line[i] && is_separator(line[i]))
		i++;
	if (is_metachar(line[i]) && (line[i] != '&' || i != 1))
		return (syntax_error(line + i, -1));
